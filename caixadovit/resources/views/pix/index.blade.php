...
<div class="container-fluid my-4">
    <div style="margin-bottom: 15px; padding-top:15px; display:flex; justify-content:space-between; align-items:center;">
        <h2>Vendas no <i>Pix <h5>R$ {{ number_format($diferencaTotal, 2, ',', '.') }}</h5></i></h2> 
        <a href='{{route('pix.novo')}}' class='btn btn-success' style="height: 40px;"><i class="fas fa-coins"></i> &nbsp Cadastrar</a>
    </div>

    <!-- Formulário de busca -->
    <form action="{{ route('pix.buscar') }}" method="GET" class="mb-4">
        <div class="row">
            <div class="col-md-3">
            ...
            <div class="col-md-2">
                ...
                <a href="{{ route('pix') }}" class="btn btn-secondary">Limpar Filtros</a>
            </div>
        </div>
    </form>
                    ...
                    <table class="table table-bordered table-striped table-full-width">
                        ...
                        <tbody>
                            @foreach ($dados as $dado)
                                @php
                                    $linkReadMore = url('/pix/' . $dado->id);
                                    $linkEditItem = url ('/pix/editar/' . $dado->id);
                                    $linkRemoveItem = url ('/pix/remover/' . $dado->id);
                                @endphp
